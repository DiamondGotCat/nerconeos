...
- 描画先はUEFIが渡してくれるFramebufferを利用する。
    - ただしFramebufferは高解像度化やマルチモニタ化などはできないため、通常は`nerconeos-display`を利用する。
- できるだけAIに頼らない
...
- ちゃんとモジュール分けする
    ...
    - `nerconeos-display`: ディスプレイマネージャー。ここで実装済みのものはFramebufferの代わりにそちらを使用する。つまり、nerconeos-displayが対応していればそのまま使用し、していない場合は外部ドライバやNKExtを試し、それでも無理であればFramebufferを利用する。
